...
function makeReq(opts: { url?: string; body?: unknown; origin?: string } = {}): any {
  ...
  req.url = opts.url ?? '/';
  req.headers = { host: 'world-of-aphasya.pixlland.com', ...(opts.origin ? { origin: opts.origin } : {}) };
...
const NATIVE_STATE_REDIRECT = `worldofaphasya://discord-auth?challenge=${NATIVE_CHALLENGE}`;
...
describe('GET /api/discord (status)', () => {
  it('reports unlinked with zeroed rewards', async () => {
    ...
    expect(data.inviteUrl).toBe('https://discord.com/invite/worldofaphasya');
  });
...
describe('GET /api/auth/discord/callback', () => {
  ...
  it('returns a native returning user through a one-time handoff code', async () => {
    ...
    expect(location.protocol).toBe('worldofaphasya:');
