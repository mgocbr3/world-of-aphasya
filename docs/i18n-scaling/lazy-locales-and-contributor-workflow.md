...
## Addendum (2026-07-14): two closed decisions reopened and reversed

Both Section 4.5 decisions on the two global aggregate artifacts were reopened with owner approval on 2026-07-14 (toolchain-modernization packet, decision D4; the packet was later closed out and removed from the repo, its summary lives on GitHub issue #1868) and reversed:

- **`src/ui/i18n.resolved.sha256` is deleted outright.** No committed baseline exists anymore. `scripts/i18n_resolved_hash.mjs` survives as a print-only diagnostic (`npm run i18n:hash` prints locales, bytes, and sha256 for ad-hoc comparison); the `--write` and `--check` flags and the re-baseline flow are gone. Determinism and byte-equivalence are enforced by the committed line-item locale slices (`src/ui/i18n.resolved.generated/` and `src/admin/i18n.resolved.generated/`, which stay committed), the CI freshness diff over those slices, and the determinism tests.
- **`src/ui/i18n.status.summary.json` is gitignored, not committed.** It is still generated by `scripts/i18n_scan.mjs`, but the audit trail moved out-of-band: a CI step in both jobs posts the coverage counts (keys, translated, pending, blocked, per-locale rollup) to the GitHub job summary via `scripts/i18n_coverage_summary.mjs`.

Rationale: the two files were global aggregates whose correct merged value no textual merge can produce, so every pair of concurrent key-adding PRs conflicted on them, while the committed line-item slices auto-merge byte-perfectly and already carry the determinism and freshness signal. Generated-artifact policy going forward: committed generated artifacts must be line-item (sorted, one item per line, no counts, hashes, or timestamps); global aggregates are never committed.

The rest of this document is retained as the historical design record. Mentions below of the SHA baseline, `i18n:hash -- --check` / `-- --write`, or a committed status summary are historical; dated notes mark the places a reader could otherwise mistake for the current flow.
...
6. **Preserve every existing guarantee at equal-or-greater strength:** determinism (byte-reproducible resolved output, the SHA baseline; 2026-07-14, D4: the baseline is deleted, and this guarantee now rides on the committed line-item slices, their CI freshness diff, and the determinism tests), the two-tier gate (PR English-legal, release `pending=0`), tsc key-completeness per locale, and the sim/server S3 matcher guard.
...
(2026-07-14, D4 note: this table records the 2026-06-17 state; `src/ui/i18n.resolved.sha256` has since been deleted outright and `src/ui/i18n.status.json` gitignored. See the addendum.)
...
- `scripts/i18n_resolved_hash.mjs` emits/checks the committed `src/ui/i18n.resolved.sha256` baseline. (2026-07-14, D4: now a print-only diagnostic; the committed baseline is deleted. See the addendum.)
...
- The barrel composes `en` exactly as today and is the only place `EnTranslations` / `TranslationKey` / `Leaves` are defined, so `Leaves<typeof en, 6>` is still computed over one assembled `en`. (Note, 2026-07-14: `TranslationKey` now re-exports the build-generated flat union `translation_keys.generated.ts` instead of computing `Leaves<typeof en, 6>`, decision D2; the barrel remains the single definition site, so this section's conclusion stands.)
...
| **Maintainer** (Fernando) | Fill all non-English overlays before release via `npm run i18n:worklist`; regenerate; ship from `release/**`. (The SHA re-baseline step was retired 2026-07-14, D4, with the baseline itself.) | n/a |
...
The maintainer section uses the real script names: `npm run i18n:worklist` (mapped to `scripts/i18n_fill_worklist.mjs`), `npm run i18n:build`, `npm run i18n:scan`, `npm run i18n:admin`. (2026-07-14, D4: the release sequence's SHA re-baseline step is removed; the baseline is deleted and `i18n:hash` is print-only.)
...
(2026-07-14, D4 note: the `i18n:hash -- --check` verifications in the steps below predate the deletion of the committed SHA baseline and are retained as written history. For any step still to be implemented, read each of them as: the committed `i18n.resolved.generated/` directories regenerate byte-identically (`git diff --exit-code`) and the determinism tests pass; `npm run i18n:hash` remains available as a print-only diagnostic for ad-hoc comparison. See the addendum.)
...
| `tests/i18n_overlay_key_membership.test.ts` | per-locale **overlays** `src/ui/i18n.locales/<lang>.ts` | Overlays are unchanged by Phase 3; unaffected by either option. (2026-07-14: test retired by the flat-TranslationKey change, D2; row kept for the historical record.) |
...
| `tests/i18n_resolved_equivalence.test.ts` | Keep the SHA `--check` and hash-equality tests unchanged (the hash reads `i18n.ts` exports, split-agnostic). Repoint the `git ls-files --error-unmatch` and `git diff --exit-code` checks at the `src/ui/i18n.resolved.generated/` directory. Optionally add an `assertDeterministic` double-gen check. (2026-07-14, D4: the SHA `--check` and hash-equality tests are retired with the deleted baseline; the directory tracking and `git diff --exit-code` freshness checks plus the determinism check carry byte-equivalence.) |
...
**CI (`.github/workflows/ci.yml`):** add a `Generate i18n artifacts` step (`npm run i18n:gen`) to both the PR-tier and release-tier jobs, after `npm ci` and before the typecheck and build. (Added 2026-07-14, D4: both jobs also post the registry coverage counts, keys, translated, pending, blocked, and the per-locale rollup, to the GitHub job summary via `scripts/i18n_coverage_summary.mjs`; this out-of-band step is the audit trail that replaced the committed summary.) The release-tier job keeps `I18N_RELEASE_TIER=1`; generators do not branch on it. No `manualChunks` change in `vite.config.ts` (Rollup auto-splits on the `loaders.ts` `import()` thunks). No `server/static_cache.ts` change (existing `/assets/*` immutable policy covers the new chunks).
...
**Determinism and two-tier guarantees, proven.** The SHA baseline (git-independent, read from `i18n.ts` exports) plus tsc (committed-status-independent) plus double-generation byte-equality cover determinism at equal-or-greater strength than today. (2026-07-14, D4: with the baseline deleted, determinism is carried by the double-generation check, the committed slices' `git diff --exit-code` freshness gate, and tsc. See the addendum.) The two-tier guarantee lives entirely in test assertions gated on `process.env.I18N_RELEASE_TIER === "1"` and the CI job split by git ref; none of those touch the committed/gitignored status of any artifact, so gitignoring `status.json` changes nothing about what is asserted or when.
...
1. `npm run i18n:hash -- --check` prints OK against the unchanged baseline at every step. (Amended 2026-07-14, D4: the baseline and `--check` are gone; the equivalent gate is that the committed `i18n.resolved.generated/` directories regenerate byte-identically, `git diff --exit-code`, at every step, with `npm run i18n:hash` available for ad-hoc bytes/sha256 comparison.)
...
| `src/ui/i18n.status.json` | 4.74 MB | 187,366 | generated (build/test-only) | severe | Gitignore; regenerated by `pretest` `i18n:scan`; optional committed summary (2026-07-14, D4: the summary is generated but gitignored; see the addendum). |
| `src/ui/i18n.resolved.sha256` | 65 B | 1 | generated baseline | none | Keep committed; the determinism anchor. (Reversed 2026-07-14, D4: deleted outright; no committed baseline exists. See the addendum.) |
