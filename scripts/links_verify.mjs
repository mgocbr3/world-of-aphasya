...
async function main() {
  ...
  try {
    ...
    ok(title === 'World of Aphasya - Official Links', `document.title is localized ("${title}")`);
    ...
    for (const want of EXPECTED_LINKS) {
      ok(
        links.some((l) => l.href === want),
        `link present: ${want}`,
      );
    }
    ok(
      links.every((l) => l.target === '_blank'),
      `every link target=_blank`,
    );
    ok(
      links.every(
        (l) => (l.rel || '').includes('noopener') && (l.rel || '').includes('noreferrer'),
      ),
      `every link rel=noopener noreferrer`,
    );
    ok(
      links.every((l) => l.hasNewTabHint),
      `every link has a "(opens in new tab)" hint`,
    );
    ...
    ok(
      ...
      `focus-visible outline on link (${outline.width} ${outline.style})`,
    );
    ...
    const i18nMissing = await page.evaluate(() => {
      ...
      for (const el of document.querySelectorAll(
        '[data-i18n],[data-i18n-html],[data-i18n-alt],[data-i18n-aria]',
      )) {
    ...
    const tap = await page.evaluate(() => {
      const rects = Array.from(document.querySelectorAll('a.btn')).map(
    ...
    await page.reload({ waitUntil: 'networkidle2' });
    const nOverflow = await page.evaluate(
      () => document.documentElement.scrollWidth - document.documentElement.clientWidth,
    );
