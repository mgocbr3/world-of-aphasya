...
//
// Usage:
//   node scripts/assets/fenbridge_town/export_fenbridge_town.mjs
//   node scripts/assets/fenbridge_town/export_fenbridge_town.mjs --raw-only
//   node scripts/assets/fenbridge_town/export_fenbridge_town.mjs --preview-only
//   node scripts/assets/fenbridge_town/export_fenbridge_town.mjs --preview-only --asset gate_arch
//   node scripts/assets/fenbridge_town/export_fenbridge_town.mjs --verify-staged --no-preview
...
import { fenbridgeTownSourceFingerprint } from './source_fingerprint.mjs';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(HERE, '..', '..', '..');
const ENTRY = path.join(HERE, 'export_entry.js');
const SPEC = path.join(ROOT, 'scripts/assets/specs/fenbridge_town.json');
...
const BUILD_SUPPORT_MAPS = path.join(HERE, 'build_support_maps.mjs');
const RAW_ROOT = path.join(ROOT, 'tmp/asset_src/fenbridge_town');
const TEMP_ROOT = path.join(ROOT, 'tmp/asset_optimized/fenbridge_town');
const PREVIEW_ROOT = path.join(ROOT, 'tmp/fenbridge_town_preview');
const DEFAULT_EVIDENCE_ROOT = path.join(ROOT, 'docs/screenshots/fenbridge-rebuild/assets');
const REFERENCE_ROOT = path.join(ROOT, 'docs/design/fenbridge-rebuild/references');
const SUPPORT_MAP_PATHS = Object.freeze([
  path.join(ROOT, 'public/textures/fenbridge_surface_atlas.webp'),
  path.join(ROOT, 'public/textures/fenbridge_surface_normal.webp'),
  path.join(ROOT, 'public/textures/fenbridge_surface_roughness.webp'),
]);
...
if (previewAsset && !FENBRIDGE_TOWN_ASSET_IDS.includes(previewAsset)) {
  throw new Error(`unknown Fenbridge town preview asset: ${previewAsset}`);
}
const previewAssetIds = previewAsset ? [previewAsset] : FENBRIDGE_TOWN_ASSET_IDS;
...
function verifyGlbContract(stats, { optimized, sourceFingerprint }) {
  const contract = FENBRIDGE_TOWN_CONTRACTS[stats.assetId];
  const emissiveMaterials = stats.materials.filter((material) =>
    material.emissive.some((value) => value > 0),
  );
  const expectedUsed = [
    ...(optimized ? ['EXT_meshopt_compression', 'KHR_mesh_quantization'] : []),
    ...(emissiveMaterials.length > 0 ? ['KHR_materials_emissive_strength'] : []),
  ].sort();
  ...
  assertCondition(
    stats.meshes.length >= 1 && stats.meshes.length <= 2,
    `${stats.path} must contain one or two meshes`,
  );
  assertCondition(
    stats.primitives === stats.meshes.length,
    `${stats.path} must contain one primitive per mesh`,
  );
  assertCondition(
    stats.meshes.every(
      (mesh) =>
  ...
  assertCondition(
    stats.materials.length === stats.meshes.length,
    `${stats.path} must contain one material per mesh`,
  );
  const materialNames = stats.materials.map((material) => material.name).sort();
  assertCondition(
    JSON.stringify(materialNames) === JSON.stringify(['FenbridgeOpaque']) ||
      JSON.stringify(materialNames) === JSON.stringify(['FenbridgeEmissive', 'FenbridgeOpaque']),
    `${stats.path} material names changed`,
  );
  assertCondition(
    emissiveMaterials.length <= 1,
    `${stats.path} contains more than one emissive material`,
  );
  if (optimized) {
    assertCondition(
  ...
  assertCondition(runtime?.coordinateFrame?.up === '+Y', `${stats.path} up axis changed`);
  assertCondition(runtime?.coordinateFrame?.right === '+X', `${stats.path} right axis changed`);
  ...
  for (const [index, definition] of contract.sockets.entries()) {
    ...
    assertApproxArray(socket.translation, definition.position, `${definition.name} translation`);
...
function runOptimizer(outputRoot) {
  ...
  if (pipeline.status !== 0) {
    throw new Error(`Fenbridge town optimizer failed with status ${pipeline.status ?? 'unknown'}`);
  }
}

function runCheckedNodeScript(script, args, label) {
...
function verifySupportMaps() {
  runCheckedNodeScript(BUILD_SUPPORT_MAPS, ['--check'], 'Fenbridge support-map check');
  const total = SUPPORT_MAP_PATHS.reduce((sum, mapPath) => sum + statSync(mapPath).size, 0);
  assertCondition(
    total <= FENBRIDGE_TOWN_WAVE_CEILINGS.supportTextureBytes,
    `Fenbridge support maps exceed ${FENBRIDGE_TOWN_WAVE_CEILINGS.supportTextureBytes} bytes`,
  );
  return total;
}

function runTextureCompression(paths) {
  runCheckedNodeScript(
    COMPRESS_TEXTURES,
    paths.map((filePath) => path.relative(ROOT, filePath)),
    'Fenbridge KTX2 final pass',
  );
}
...
function intakeSlug(assetId) {
  return assetId.replaceAll('_', '-');
}

function writeAiReviewScaffold(assetId, sourceFingerprint) {
  const outPath = path.join(evidenceRoot, `${assetId}-ai-review.json`);
  if (existsSync(outPath)) return outPath;
  const contract = FENBRIDGE_TOWN_CONTRACTS[assetId];
  const sculptSpecPath = path.join(
    ROOT,
    'docs/design/fenbridge-rebuild/img2threejs',
    intakeSlug(assetId),
    'sculpt-spec.json',
  );
  const sculptSpec = JSON.parse(readFileSync(sculptSpecPath, 'utf8'));
  const targets = Array.isArray(sculptSpec.featureReviewTargets)
    ? sculptSpec.featureReviewTargets
    : [];
  const features = targets.map((target) => ({
    id: target.id,
    name: target.name,
    tier: target.tier,
    minimumScore: Math.max(0.7, target.minimumScore ?? 0.7),
    mustPass: target.mustPass !== false,
    visible: null,
    score: null,
    notes: '',
  }));
  assertCondition(features.length > 0, `${assetId} sculpt spec has no feature review targets`);
  const manifest = {
    schemaVersion: 1,
    assetId,
    sourceFingerprint,
    reference: path.relative(ROOT, path.join(REFERENCE_ROOT, contract.referenceName)),
    minimumOverallScore: 0.7,
    requiredViews: ['front', 'right', 'rear', 'left', 'hero', 'player-scale', 'collider-overlay'],
    evidence: {
      rawContact: `${assetId}-raw-contact.png`,
      optimizedContact: `${assetId}-optimized-contact.png`,
      optimizedAuditContact: `${assetId}-optimized-audit-contact.png`,
      referenceComparison: `${assetId}-comparison.png`,
    },
    features,
    overall: { score: null, decision: 'pending-ai-review', reviewer: null, notes: '' },
    intentionalOmissions: ['inaccessible interior geometry', 'shipping collision mesh'],
  };
  mkdirSync(path.dirname(outPath), { recursive: true });
...
async function renderSerializedContacts(kind, paths, assetIds = FENBRIDGE_TOWN_ASSET_IDS) {
      ...
      const renderViews = async (views, subdirectory = '') => {
        ...
        for (const view of views) {
          const rendered = await page.evaluate(
            ({ data, id, viewName }) =>
              window.renderFenbridgeTownSerializedPreview(data, id, viewName),
            { data: base64, id: assetId, viewName: view },
          );
          assertCondition(
            rendered.evidenceSurface.textureBindings > 0 &&
              rendered.evidenceSurface.textureBindings === rendered.evidenceSurface.pbrBindings,
            `${assetId}:${view} preview missed an atlas material binding`,
          );
          assertCondition(
            rendered.evidenceSurface.pbrBindings > 0,
            `${assetId}:${view} preview missed its normal/roughness bindings`,
          );
          assertCondition(
            rendered.evidenceSurface.uvBounds.span.every((span) => span > 0),
            `${assetId}:${view} preview produced a zero-area UV projection`,
          );
          ...
          console.log(
            `${kind} preview: ${assetId} ${view} ${JSON.stringify({
              ...
              evidenceSurface: rendered.evidenceSurface,
            })}`,
          );
        }
        return files;
...
const previewSupportMapData = Object.fromEntries(
  ['base', 'normal', 'roughness'].map((key, index) => [
    key,
    `data:image/webp;base64,${readFileSync(SUPPORT_MAP_PATHS[index]).toString('base64')}`,
  ]),
);
const html =
  '<!doctype html><html><head><meta charset="utf-8"></head><body>' +
  `<script>window.__fenbridgeSupportMapData=${JSON.stringify(previewSupportMapData)}</script>` +
  `<script>${bundle}</script></body></html>`;
const supportTextureBytes = verifySupportMaps();
const sourceFingerprint = fenbridgeTownSourceFingerprint(ROOT);
if (!noPreview) {
  for (const assetId of previewOnly ? previewAssetIds : FENBRIDGE_TOWN_ASSET_IDS) {
    const scaffold = writeAiReviewScaffold(assetId, sourceFingerprint);
    console.log(`AI-review scaffold: ${path.relative(ROOT, scaffold)}`);
  }
}
...
try {
  ...
  for (const assetId of previewOnly ? previewAssetIds : FENBRIDGE_TOWN_ASSET_IDS) {
    if (!previewOnly) {
      const result = await page.evaluate((id) => window.exportFenbridgeTownAsset(id), assetId);
      const repeat = await page.evaluate((id) => window.exportFenbridgeTownAsset(id), assetId);
      const firstBytes = Buffer.from(result.b64, 'base64');
      const repeatBytes = Buffer.from(repeat.b64, 'base64');
      assertCondition(
        firstBytes.equals(repeatBytes),
        `${assetId} raw browser export is nondeterministic: ` +
          `${createHash('sha256').update(firstBytes).digest('hex')} != ` +
          `${createHash('sha256').update(repeatBytes).digest('hex')}`,
      );
      const out = rawPath(assetId);
      mkdirSync(path.dirname(out), { recursive: true });
      writeFileSync(out, firstBytes);
      console.log(`raw: ${path.relative(ROOT, out)}`);
      console.log(`deterministic raw rebuild: ${assetId}`);
    ...
    if (!noPreview) {
      ...
      for (const view of TURNAROUND_VIEWS) {
        const stats = await page.evaluate(
          ({ id, viewName }) => window.renderFenbridgeTownPreview(id, viewName),
...
if (previewOnly) {
  ...
  for (const assetId of previewAssetIds) {
    ...
    await makeComparisonSheet(
      path.join(REFERENCE_ROOT, contract.referenceName),
      hero,
      comparison,
...
if (!previewOnly) {
  ...
  assertCondition(
    rawStats.reduce((sum, stats) => sum + stats.triangles, 0) <=
      FENBRIDGE_TOWN_WAVE_CEILINGS.uniqueTriangles,
    `town raw assets exceed ${FENBRIDGE_TOWN_WAVE_CEILINGS.uniqueTriangles} unique triangles`,
  );
  const weightedRawTriangles = rawStats.reduce(
    (sum, stats) => sum + stats.triangles * FENBRIDGE_TOWN_CONTRACTS[stats.assetId].placementCount,
    0,
  );
  assertCondition(
    weightedRawTriangles <= FENBRIDGE_TOWN_WAVE_CEILINGS.placementWeightedTriangles,
    `town placement-weighted raw assets exceed ` +
      `${FENBRIDGE_TOWN_WAVE_CEILINGS.placementWeightedTriangles} triangles`,
  );
  ...
  if (!rawOnly) {
    ...
    const optimizedPaths = new Map();
    for (const assetId of FENBRIDGE_TOWN_ASSET_IDS) {
      optimizedPaths.set(assetId, outputPath(optimizedRoot, assetId));
    }
    runTextureCompression([...optimizedPaths.values()]);
    const optimizedStats = [];
    for (const assetId of FENBRIDGE_TOWN_ASSET_IDS) {
      const glbPath = optimizedPaths.get(assetId);
      const stats = await inspectGlb(glbPath, assetId);
    ...
    const optimizedBytes = optimizedStats.reduce((sum, stats) => sum + stats.bytes, 0);
    assertCondition(
      optimizedBytes <= FENBRIDGE_TOWN_WAVE_CEILINGS.glbBytes,
      `town optimized assets exceed ${FENBRIDGE_TOWN_WAVE_CEILINGS.glbBytes} GLB bytes`,
    );
    assertCondition(
      optimizedBytes + supportTextureBytes <= FENBRIDGE_TOWN_WAVE_CEILINGS.totalMediaBytes,
      `Fenbridge media exceeds ${FENBRIDGE_TOWN_WAVE_CEILINGS.totalMediaBytes} bytes`,
    );
    assertCondition(
      optimizedStats.reduce((sum, stats) => sum + stats.triangles, 0) <=
        FENBRIDGE_TOWN_WAVE_CEILINGS.uniqueTriangles,
      `town optimized assets exceed ${FENBRIDGE_TOWN_WAVE_CEILINGS.uniqueTriangles} unique triangles`,
    );
    const weightedOptimizedTriangles = optimizedStats.reduce(
      (sum, stats) =>
        sum + stats.triangles * FENBRIDGE_TOWN_CONTRACTS[stats.assetId].placementCount,
      0,
    );
    assertCondition(
      weightedOptimizedTriangles <= FENBRIDGE_TOWN_WAVE_CEILINGS.placementWeightedTriangles,
      `town optimized placement-weighted assets exceed ` +
        `${FENBRIDGE_TOWN_WAVE_CEILINGS.placementWeightedTriangles} triangles`,
    );
    ...
    if (verifyStaged) {
      ...
      runTextureCompression(
        FENBRIDGE_TOWN_ASSET_IDS.map((assetId) => outputPath(repeatRoot, assetId)),
      );
      for (const assetId of FENBRIDGE_TOWN_ASSET_IDS) {
    ...
    if (!noPreview) {
      ...
      assertCondition(
        rawContacts?.size === FENBRIDGE_TOWN_ASSET_IDS.length,
