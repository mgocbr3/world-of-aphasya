...
#
# World of Aphasya ships a pre-push QA floor in .githooks/. Git does not use a checked-in
...
if [ -z "$current" ]; then
  if git config --local core.hooksPath .githooks 2>/dev/null; then
    echo "World of Aphasya: enabled .githooks (pre-push QA floor: tsc, guard tests, biome, copy rules). Undo with: git config --unset core.hooksPath" >&2
  fi
else
  ...
  case "$current" in
    /*/.githooks)
      if [ "$current" != "$dir/.githooks" ]; then
        echo "World of Aphasya: core.hooksPath points at another checkout's .githooks ($current); the pre-push floor that runs here may be a stale copy. Repoint with: git config --local core.hooksPath .githooks" >&2
      fi
      ;;
  esac
fi
