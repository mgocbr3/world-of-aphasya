// The Epic achievement mirror: an OBSERVER, never an authority. The sim
// decides unlocks, server/deeds_records.ts persists them into character_deeds,
// and this module copies a linked account's unlocks outward to Epic via the
// server-trusted Connect + Stats Achievements Web API path (O2). Nothing here can grant, deny, or reorder a deed, and the
...
//
// Delivery model: at-least-once with in-flight dedupe. Epic achievement
// unlocks are idempotent for an already-unlocked achievement, so a
...
// canonical and Epic is a mirrored subset), and TWO reconcile paths replay
...
//
// Secrets: the client secret is read inside web_api.ts request builders only;
// no log line here carries a URL, a body, an Authorization header, or anything upstream echoed.

import { earnedDeedIdsForAccount } from '../deeds_db';
import { ACHIEVEMENT_MAP } from './achievement_map';
import { epicClientId, epicClientSecret, epicDeploymentId, epicEnabled } from './config';
import { epicLinkForAccount } from './epic_db';
...
/** Most unlock names coalesced into one Stats Achievements unlock call. A whole
 ...
 *  rest of the queue. Epic looking down must not grind every queued account
 *  through the full retry ladder; reconcile-on-login is the documented heal. */
export const OUTAGE_TRIP_BATCHES = 2;
/** Default shutdown drain deadline when stopEpicMirror is called without one. */
export const SHUTDOWN_DEADLINE_MS = 5000;
/** How long a link lookup is trusted before re-reading epic_links. */
...
interface MirrorDeps {
  linkForAccount(accountId: number): Promise<{ epicAccountId: string } | null>;
...
const REAL_DEPS: MirrorDeps = {
  linkForAccount: (accountId) => epicLinkForAccount(accountId),
...
export function setEpicMirrorDepsForTests(overrides: Partial<MirrorDeps>): void {
  deps = { ...REAL_DEPS, ...overrides };
}

// ---------------------------------------------------------------------------
// Link cache. Per-process, short TTL, promise-valued so a retro burst of
// unlocks for one account does exactly one epic_links read on the lookup
...
// flip at all. The actual revocation barrier is the fresh epic_links read
// attemptPushBatch does before every push.
// ---------------------------------------------------------------------------

interface LinkCacheEntry {
  epicAccountId: Promise<string | null>;
  expiresAt: number;
}
...
function cachedEpicAccountId(accountId: number): Promise<string | null> {
  const now = deps.now();
  const hit = linkCache.get(accountId);
  if (hit && hit.expiresAt > now) return hit.epicAccountId;
  const epicAccountId = deps
    .linkForAccount(accountId)
    .then((row) => row?.epicAccountId ?? null)
    .catch(() => {
  ...
  linkCacheSet(accountId, { epicAccountId, expiresAt: now + LINK_CACHE_TTL_MS });
  return epicAccountId;
}

/** The link and unlink routes call this the moment epic_links changes, so
 *  the mirror's view flips in the same request, not a TTL later. */
export function onLinkChanged(accountId: number, epicAccountId: string | null): void {
  linkCacheSet(accountId, {
    epicAccountId: Promise.resolve(epicAccountId),
...
// (account, epicAccountId, achievement) triple collapses while one is already on
// its way. The account id is part of the key: when an Epic account moves
...
// with every other queued item for the same (accountId, epicAccountId) into one
// unlock call (up to MAX_BATCH_NAMES). A 72-deed reconcile for one
...
// Consecutive exhausted batches (Epic unreachable), reset by any delivered
...
function pushKey(item: PushItem): string {
  return `${item.accountId}:${item.epicAccountId}:${item.achName}`;
}

// How a batch resolved: 'delivered' (Epic took it, heal the trip-wire),
...
async function runDrain(): Promise<void> {
  try {
    ...
    while (queue.length > 0) {
      ...
      try {
        result = await attemptPushBatch(head.accountId, head.epicAccountId, batch);
      } catch {
        ...
        console.warn(
          `epic mirror: dropping ${batch.length} unlock(s), push attempt threw unexpectedly`,
        );
        result = 'exhausted';
      } finally {
      ...
      if (result === 'delivered') {
        ...
        if (consecutiveExhausted >= OUTAGE_TRIP_BATCHES && queue.length > 0) {
          // Trip-wire: Epic looks down. Fast-drop everything still queued with
          // one summary line rather than grinding each batch through the full
          // ladder; reconcile-on-login replays the durable outbox once Epic is
          // back. The counter stays tripped until a push succeeds.
          console.warn(
            `epic mirror: Epic unreachable after ${consecutiveExhausted} exhausted batches, fast-dropping ${queue.length} queued unlock(s)`,
          );
...
// Group the head with every other queued item sharing its (accountId, epicAccountId),
...
function takeBatch(head: PushItem): PushItem[] {
  ...
  for (let i = 0; i < queue.length && batch.length < MAX_BATCH_NAMES; ) {
    ...
    if (candidate.accountId === head.accountId && candidate.epicAccountId === head.epicAccountId) {
...
async function attemptPushBatch(
  ...
  if (clientId === null || clientSecret === null || deploymentId === null) {
    console.warn(
      'epic mirror: enabled without EPIC_CLIENT_ID/EPIC_CLIENT_SECRET/EPIC_DEPLOYMENT_ID, dropping unlock',
    );
    return 'dropped';
  }
  const achNames = batch.map((it) => it.achName);
  // Push-time revalidation, the revocation barrier: a fresh epic_links read
  // (deliberately not the TTL cache, which a peer realm process cannot see
  // invalidated) must still name this exact Epic account id, or the batch was
  // queued by a read that lost a race with an unlink and is dropped whole.
  // Comparing ids rather than row existence keeps queued pushes flowing after
  // a relink. A REJECTED read proves nothing about the link, so it is retried
  // once (one transient DB blip must not eat a push), and a second rejection
  // drops WITH a warn line so operators can see the loss instead of a silent
  // gap; a reconcile (on link or on login) heals it either way.
  //
  ...
  for (let attempt = 1; attempt <= MAX_PUSH_ATTEMPTS; attempt++) {
    let row: { epicAccountId: string } | null;
    try {
      ...
      try {
        ...
        console.warn(
          `epic mirror: dropping ${batch.length} unlock(s), link revalidation read failed twice`,
        );
        return 'dropped';
      }
    }
    if (row?.epicAccountId !== epicAccountId) return 'dropped';
    const ok = await deps.pushUnlocks({
      clientId,
      clientSecret,
      deploymentId,
      epicAccountId,
    ...
    // beats the deadline. The residual is reported by stopEpicMirror and healed
  ...
  // One fixed line, no URL, no body, no secret: a reconcile heals the gap.
  console.warn(
    `epic mirror: dropping ${batch.length} unlock(s) after ${MAX_PUSH_ATTEMPTS} attempts`,
  );
  return 'exhausted';
}
...
export function onDeedRecorded(accountId: number, deedId: string): void {
  try {
    if (!epicEnabled()) return;
    const achName = ACHIEVEMENT_MAP[deedId];
    if (achName === undefined) return;
    void cachedEpicAccountId(accountId)
      .then((epicAccountId) => {
        if (epicAccountId !== null) enqueue({ accountId, epicAccountId, achName });
      })
      .catch(() => {});
  } catch (err) {
    console.error('epic mirror: onDeedRecorded failed:', err);
  }
}

/**
 * Reconcile-on-link: push everything the account already earned, intersected
 * with the map, to the freshly linked Epic account. Fire-and-forget; the link
 * response never waits on it. The server store is canonical and Epic is a
    ...
    void deps
      ...
      .then((deedIds) => {
        for (const deedId of deedIds) {
          const achName = ACHIEVEMENT_MAP[deedId];
          if (achName !== undefined) enqueue({ accountId, epicAccountId, achName });
        }
      })
      .catch((err) => {
        console.error('epic mirror: reconcile read failed:', err);
      });
  } catch (err) {
    console.error('epic mirror: reconcileLink failed:', err);
  }
}
 ...
 * earned-and-mapped set to its currently linked Epic id, idempotent Epic-side.
 *
...
export function reconcileOnLogin(accountId: number): void {
  try {
    ...
    // Read the link DIRECTLY (not through cachedEpicAccountId, which collapses a read
    ...
    void deps
      .linkForAccount(accountId)
      .then((row) => {
        const epicAccountId = row?.epicAccountId ?? null;
        if (epicAccountId === null) return;
      ...
      .catch((err) => {
        ...
        console.error('epic mirror: login reconcile failed:', err);
      });
  } catch (err) {
    console.error('epic mirror: reconcileOnLogin failed:', err);
  }
}

/** The current drain tail, for tests to await deterministic queue settling. */
export function epicMirrorIdle(): Promise<void> {
  return drain;
}
 ...
 */
export async function stopEpicMirror(deadlineMs?: number): Promise<void> {
  shuttingDown = true;
  const tail = drain;
  let timedOut = false;
  ...
  if (timedOut) {
    ...
    if (residual > 0) {
      console.warn(
        `epic mirror: shutdown deadline reached, ${residual} unlock(s) undelivered (reconcile heals them)`,
      );
    }
  }
}
...
export function resetEpicMirrorForTests(): void {
  queue.length = 0;
  pending.clear();
  linkCache.clear();
